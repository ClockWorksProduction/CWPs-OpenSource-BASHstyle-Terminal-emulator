...
class CentralTerminal {
    ...
    async boot() {
      ...
      this.bootRegistry.add(new BootCheck('Loading saved session', () => {
        if (this.preventSessionLoad) return true; // Skip load if VFS is already populated

        try {
          const vosData = localStorage.getItem('cterm_vos');
          if (vosData) {
            const parsedData = JSON.parse(vosData);
            if (Object.keys(parsedData).length > 0) {
                 this.vOS = VOS.fromJSON(parsedData);
            }
          }
          ...
          // If saved data is corrupt, clear it to prevent boot loops
          localStorage.removeItem('cterm_vos');
          localStorage.removeItem('cterm_history');
    ...
    _registerDefaultCommands() {
      ...
      this.addCommand(cmd('tree', 'show directory tree', args => {
        const path = args[0] || '.';
        const dir = this.vOS.resolve(path);
        if (!(dir instanceof VDirectory)) { this._print(`tree: ${path}: No such directory`); return; }
        this._print(this.vOS.pathOf(dir)); // Use pathOf to print the full path
        this._tree(dir, '');
      }));
